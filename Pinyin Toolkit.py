...
def updatefact(fact, expression):
    ...
    def formatmeanings(meanings):
        ...
        meaningcount =len(meanings)

        # Delete entries that match certain conditions
        ...
        # Add the pretty entry numbers to start of each entry
        if meaningcount > 1 and numbermeanings != None:
            def meaningnumber(n):
                if n < meaningcount):
...
def measurewordmeaning(meaning):
    ...
    if meaning.startswith("CL:"):
        ...
        # method: find the divider between simplified and traditional (i.e. "|") then go to left or right, cutting the MW and put the remainder of the text on end
